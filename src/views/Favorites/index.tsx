...
import { CharacteresList, Container, Content, Title } from "./styles";
  ...
  async function fetchFavoritesCharacters() {
    try {
      const result = await axios.get(
        `https://rickandmortyapi.com/api/character/${convertStringValue}`
      );
      const resultdata = result.data;

      if (!resultdata) return console.log("não tem dado");

      if (resultdata.length > 1) setCharacters([...characters, ...resultdata]);
      else setCharacters((oldState) => [...oldState, resultdata]);
    } catch (error) {
      console.log(error);
    }
  }
