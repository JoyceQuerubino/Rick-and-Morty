...
export default function Character() {
  ...
  const [charactersIdAsyncStorageList, setCharactersIdAsyncStorageList] =
    useState("");
  ...
  async function getIdArrayCharacters() {
    ...
    if (response !== null) {
      ...
      else setBtnFavoriteStatus(false);
    }
  }
  ...
  function setIdCharacter(id: number) {
    var idConvertedToArray = charactersIdAsyncStorageList
      .split(",")
      .map(Number);
    ...
    async function fetchEpisodes() {
      try {
        const result = await axios.get(
          `https://rickandmortyapi.com/api/episode/${filteredUrl}`
        );
        const resultdata = result.data;

        if (!resultdata) return console.log("Requisição concluída");

        // Quando tiver apenas 1 episódio, precisa mudar a forma de receber os dados
        if (resultdata.length > 1) setEpisode([...episode, ...resultdata]);
        else setEpisode((oldState) => [...oldState, resultdata]);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  function getEpisodeNumberFromUr(fullUrl: string) {
    const episodeNumber = fullUrl.replace(
  ...
  return (
    <Container>
      <BtnReturn title="Return" />
      <Header>
        <Image source={{ uri: pagCharacters.image }} resizeMode="cover" />
        <FavoriteContainer>
          <Title>{pagCharacters.name}</Title>
          <BtnFavorite onPress={handleFavoriteCharacter}>
            {btnFavoriteStatus ? (
